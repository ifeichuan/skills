...
这是真实面经题干汇总。drill 优先贴近这些出现过的题，尤其是带有追问链和项目背景的知识点。这里记录的是题目与面试经历，不等于经过官方验证的答案；出题后应结合 `nowcoder-hot-qa.md` 或官方文档核对。
...
- 快手 AI Agent 前端二面：Fiber、流式打字机、Markdown/LaTeX、性能指标、Webpack、微前端、状态管理、异常监控
- 百度 AI Agent 前端实习：Skill/Workflow、状态机、幂等、熔断、SSE 重连、断点续传、FCP/LCP、RAG 与 Multi-Agent
- 百度 Agent 前端一面：SSE、Markdown 安全、CSRF、任务进度、自定义事件、全局状态、性能指标
- 美团前端暑期面：SSE/WebSocket、AI 工作流、质量验收、人工介入、弱网与语音降噪
- 前端+AI 面试真题：SSE、RAG、Agent、Function Calling、MCP、Skills、多 Agent、ReAct
